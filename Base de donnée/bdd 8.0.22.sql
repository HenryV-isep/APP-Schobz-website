...
CREATE TABLE IF NOT EXISTS `Schobz_data`.`Auto_ecole` (
  ...
  UNIQUE INDEX `numero_de_telephone_UNIQUE` (`numero_de_telephone` ASC) VISIBLE,
  UNIQUE INDEX `mail_UNIQUE` (`adresse_mail` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Schobz_data`.`Personne` (
  ...
  UNIQUE INDEX `adresse_mail_UNIQUE` (`adresse_mail` ASC) INVISIBLE,
  INDEX `fk_Personne_Autoecole_idx` (`cle_etrangere_auto_ecole` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `Schobz_data`.`Test` (
  ...
  PRIMARY KEY (`id_test`),
  UNIQUE INDEX `numero_du_Test_UNIQUE` (`id_test` ASC) VISIBLE,
  UNIQUE INDEX `nom_du_test_UNIQUE` (`nom_du_test` ASC) VISIBLE,
  INDEX `fk_Test_Personne_idx` (`cle_etrangere_personne` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `Schobz_data`.`Capteurs` (
  ...
  UNIQUE INDEX `numero_du_Capteur_UNIQUE` (`id_Capteur` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Schobz_data`.`Reponse` (
  ...
  UNIQUE INDEX `reponse_UNIQUE` (`id_reponse` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Schobz_data`.`Question` (
  ...
  UNIQUE INDEX `question_UNIQUE` (`id_question` ASC) VISIBLE,
  INDEX `fk_FAQQuestion_FAQReponse_idx` (`cle_etrangere_faq_reponse` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `Schobz_data`.`Resultats` (
  ...
  UNIQUE INDEX `nom_du_test_UNIQUE` (`type` ASC) VISIBLE,
  INDEX `fk_Résultats_Capteurs_idx` (`cle_etrangere_capteur` ASC) VISIBLE,
  INDEX `fk_Résultats_Test_idx` (`cle_etrangere_Test` ASC) VISIBLE,
